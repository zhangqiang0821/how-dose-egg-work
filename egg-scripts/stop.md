...
class StopCommand extends Command {
  ...
  *run(context) {
    ...
    if (process.platform === 'win32') {
      this.logger.warn(
        'Windows is not supported, try to kill master process which command contains `start-cluster` or `--type=egg-server` yourself, good luck.'
      )
      process.exit(0)
    }
